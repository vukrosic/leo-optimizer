...
def setup_leo_optimizer(model: nn.Module, config: ModelConfig):
    """Setup Leo optimizer with hybrid approach"""
    leo_params = []
    adamw_params = []
    ...
    for name, param in model.named_parameters():
        if (param.ndim == 2 and 
            ...
            param.requires_grad):
            leo_params.append(param)
        else:
            adamw_params.append(param)

    print(f"  Leo parameters: {sum(p.numel() for p in leo_params):,}")
    print(f"  AdamW parameters: {sum(p.numel() for p in adamw_params):,}")

    leo_optimizer = Leo(leo_params, lr=config.leo_lr, betas=(0.9, 0.99), align_const=0.3)
    adamw_optimizer = torch.optim.AdamW(adamw_params, lr=config.leo_lr*0.1, weight_decay=config.weight_decay)

    return [leo_optimizer, adamw_optimizer]

def train_model(config: ModelConfig, train_loader: DataLoader, val_loader: DataLoader):
    """Train the model with Leo optimizer"""
    print(f"\n🚀 Training Small model with Leo optimizer")
    ...
    # Setup optimizers
    optimizers = setup_leo_optimizer(model, config)
